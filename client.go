...
func NewClient(authToken string, orgId string) *Client {
	config := DefaultConfig(authToken, orgId)
...
func (c *Client) fullUrl(suffix string, args ...any) string {
	return fmt.Sprintf("%s%s?GroupId=%s", c.config.BaseUrl, suffix, c.config.OrgId)
}
